 ...
 */
@WebServlet("/documentupdatesave.action")
public class DoucmentUpdateServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 获取到请求路径
				...
				d.setId(id);
				if ("documentupdatesave.action".equals(action)) {
					//上传操作
							...
							if(list!=null) {
								...
								for (FileItem fi : list) {
									...
									if(fi.isFormField()) {
										...
										d.setUser_id(user);
										//在upload文件夹下创建一个新的文件
										fileName = fileName.substring(0, index)+System.currentTimeMillis()+fileName.substring(index);
